      ...
      courses: [
        {
          ...
          lessons: [
            ...
            },
            {
              ...
              demonstration: [
                "Try it now: press `Alt+Tab` to switch to another window, then press it again to come back. Open File Explorer with `Win+E`, look at it, then close it with `Alt+F4`. Open Task Manager with `Ctrl+Shift+Esc` — notice it opens directly, no right-clicking the taskbar needed.",
                "In the terminal below, type `Get-ChildItem` and press Enter. Now press **Up Arrow** once — the command reappears without retyping. Press Enter to run it again. Next, type `cd Pro` and press **Tab** — if a folder starting with `Pro` exists, PowerShell auto-completes the name. This is **Tab completion** in action, and it becomes indispensable once you rely on it.",
                "Finally, run a few commands to fill the screen, then press `Ctrl+L` to clear the display. Your session and history are preserved — only the visual clutter is gone. These shortcuts are the building blocks of a keyboard-first workflow.",
              ],
              exerciseSteps: [
        ...
        },
        {
          ...
          lessons: [
            ...
            },
            {
              ...
              demonstration: [
                "Run `Get-Process | Sort-Object CPU -Descending | Select-Object -First 5` in the terminal. You will see the five processes consuming the most CPU, displayed with all their properties. Now refine it: `Get-Process | Sort-Object CPU -Descending | Select-Object -First 5 | Select-Object Name, CPU` — this shows only the columns you care about. Each `|` passes data to the next stage.",
          ...
          lessons: [
            {
              ...
              demonstration: [
                "Open Obsidian and create a new vault called `Engineering`. Inside it, create four folders: `Commands`, `Concepts`, `Projects`, and `Logs`. Now create a note in `Commands/` called `Get-ChildItem.md`. Write a brief description of the command, an example (`Get-ChildItem -Recurse -Filter *.log`), and add the tag `#powershell` at the bottom.",
      ...
      courses: [
        {
          ...
          lessons: [
            ...
            },
            {
              ...
              demonstration: [
                "The practice app has a broken checkout function. Running the app and clicking \"Checkout\" shows an error in the browser console: `TypeError: Cannot read properties of undefined (reading 'price')`. The first step is to reproduce this every time — click Checkout, see the TypeError. Consistent reproduction confirmed.",
                "Next, add a `console.log()` at the top of the `calculateTotal` function to inspect its input:\n```js\nfunction calculateTotal(items) {\n  console.log('items:', items);\n  // ...\n}\n```\nRefresh and click Checkout again. The console shows `items: undefined`. The bug is not in `calculateTotal` itself — it is in whatever calls this function and passes `undefined` instead of the items array.",
...
`,
            ...
            },
            {
              ...
              demonstration: [
                ...
                "The key insight: **error type + message + first application frame** gives you everything you need to start investigating. You do not need to understand the entire stack trace. Read the top, find your code, and go there.",
              ],
              exerciseSteps: [
            ...
            },
            {
              ...
              demonstration: [
                "Open the practice script. It has a function `calculateDiscount` that returns the wrong value. Set a breakpoint on the first line of the function by clicking the gutter — a red dot appears. Now press `F5` to start the debugger.",
                "The program pauses at the breakpoint. Look at the **Variables panel** on the left — it shows `price: 100` and `discount: 0.2`. Press `F10` (Step Over) to execute one line at a time. Watch the Variables panel update after each step. When `result` is calculated, you can see its value immediately — no `console.log()` needed.",
                "Now use **Step Into** (`F11`) on a line that calls a helper function. The debugger descends into that function, and the **Call Stack** panel updates to show the new call on top. You can see exactly what arguments were passed and trace the logic line by line. When done, `Shift+F11` (Step Out) returns you to the caller.",
                "The pattern: set a breakpoint where you suspect the bug, run with `F5`, inspect variables at each step with `F10`, and dive into suspicious functions with `F11`. This is faster and more reliable than inserting `console.log()` statements because you can inspect everything in real time without modifying the code.",
              ],
              exerciseSteps: [
          ...
          lessons: [
            {
              ...
              explanation: [
                ...
                "The core Docker CLI commands you will use constantly: `docker pull` downloads an image, `docker run` creates and starts a container from an image, `docker ps` lists running containers, `docker logs <container>` shows the output of a container, and `docker stop <container>` shuts it down. These five commands cover 90% of your early Docker work.",
                "Containers are **ephemeral by default** — when you stop a container, any data written inside it disappears unless you explicitly preserve it with a **volume**. This is a feature, not a bug. Ephemeral containers mean you can always start fresh. When you need data to persist (like a database), you mount a volume — but that comes in a later lesson.",
              ],
              demonstration: [
          ...
          lessons: [
            ...
            },
            {
              ...
              demonstration: [
                ...
                "When a test fails in CI, the GitHub Actions UI shows the failing step with a red X. Click into the step to see the exact error output — the same output you would see running `npm test` locally. The PR page shows a red 'checks failed' badge and the merge button is blocked (if required status checks are enabled). The developer must fix the issue, push a new commit, and wait for CI to pass before merging.",
            ...
            },
            {
              ...
              explanation: [
                ...
                "Other high-value refactoring moves: **rename** (a variable called `d` becomes `daysUntilExpiry` — the cheapest documentation you can write), **inline** (remove an unnecessary abstraction that adds complexity without value), and **simplify conditionals** (replace nested if-else chains with early returns). Every refactoring move should make the code clearer to the next person who reads it.",
              ],
              demonstration: [
                ...
                "Now a smaller refactor: rename a variable. A function has `const d = new Date()`. Rename it to `const createdAt = new Date()`. The code does the same thing, but the name communicates **intent**. Run `npm test` one more time — still green. Every refactoring step is bookended by a test run. This is the safety loop that makes refactoring professional instead of reckless.",
              ],
              exerciseSteps: [
          ...
          lessons: [
            {
              ...
              demonstration: [
                "The brief: build a CLI tool that accepts a directory path and outputs a count of files by extension. Example usage and output:\n\n```bash\nnode filecounter.ts ./src\n# .ts: 14\n# .json: 3\n# .md: 2\n# .css: 1\n# Total: 20 files\n```\n\nThe first step is parsing the argument from `process.argv[2]` and validating it — if no path is given or the path does not exist, print a clear error message and exit with a non-zero code.",
                "Break the implementation into testable pieces: one function to read the directory and collect file paths (`getFiles`), one function to count extensions from a list of file paths (`countExtensions`), and a main function that ties them together and prints the output. Write a test after each function:\n\n```typescript\ndescribe('countExtensions', () => {\n  it('counts .ts and .json files correctly', () => {\n    const files = ['a.ts', 'b.ts', 'c.json'];\n    expect(countExtensions(files)).toEqual({ '.ts': 2, '.json': 1 });\n  });\n});\n```",
            ...
            },
            {
              ...
              demonstration: [
                ...
                "Build the `POST /api/tasks` endpoint first with input validation. If `title` is missing or empty, return a `400` error:\n\n```typescript\nif (!title || title.trim() === '') {\n  return res.status(400).json({ error: 'Title is required' });\n}\n```\n\nWrite a test for this endpoint that verifies both the happy path (valid title → 201) and the error case (empty title → 400). Run `npm test` to confirm both pass before building the next endpoint.",
